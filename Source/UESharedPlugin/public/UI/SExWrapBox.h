 ...
 * the SEXWrapBox will place widgets on the next line.
 *
 * Illustration:
 *                      +-----Preferred Width
 *                      |
 ...
 *       [---]          |
 */
class  SExWrapBox : public SPanel
{
public:
	...
	SLATE_BEGIN_ARGS(SExWrapBox)
		...
		/** if true, the PreferredWidth will always match the room available to the SEXWrapBox  */
		SLATE_ARGUMENT( bool, UseAllottedWidth )

			SLATE_ARGUMENT(bool, GrowUp)
			SLATE_ARGUMENT(bool, GrowUnderOldLine)
	SLATE_END_ARGS()
...
private:
	...
	friend class SExWrapBox::FChildArranger;
};
